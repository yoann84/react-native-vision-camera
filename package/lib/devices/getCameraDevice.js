...
export function getCameraDevice(devices, position, filter = {}) {
    ...
    for (const device of filtered) {
        ...
        if (bestDevice.hardwareLevel === 'full')
            leftPoints += 4;
        if (device.hardwareLevel === 'full')
            rightPoints += 4;
        ...
        if (filter.supportsDepthCapture === true) {
            ...
            if (bestDeviceSupportsDepth)
                leftPoints += 10;
            if (deviceSupportsDepth)
                rightPoints += 10;
        }
        if (filter.physicalDevices != null) {
            ...
            for (const d of bestDevice.physicalDevices) {
                if (filter.physicalDevices.includes(d))
                    leftPoints += 1;
                else
                    leftPoints -= 1;
            }
            ...
            if (bestDevice.physicalDevices.includes('wide-angle-camera'))
                leftPoints += 2;
            if (device.physicalDevices.includes('wide-angle-camera'))
                rightPoints += 2;
            // if we have more than one device, we rank it lower. we only want a simple camera
            if (bestDevice.physicalDevices.length > device.physicalDevices.length)
                leftPoints -= 1;
            if (device.physicalDevices.length > bestDevice.physicalDevices.length)
                rightPoints -= 1;
        }
