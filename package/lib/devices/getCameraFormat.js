    ...
    for (const format of device.formats) {
        ...
        if (filter.videoResolution != null) {
            ...
            if (filter.videoResolution.target === 'max') {
                ...
                if (leftVideoResolution > rightVideoResolution)
                    leftPoints += filter.videoResolution.priority;
                if (rightVideoResolution > leftVideoResolution)
                    rightPoints += filter.videoResolution.priority;
            }
            else {
        ...
        if (filter.photoResolution != null) {
            ...
            if (filter.photoResolution.target === 'max') {
                // We just want the maximum resolution
                if (leftPhotoResolution > rightPhotoResolution)
                    leftPoints += filter.photoResolution.priority;
                if (rightPhotoResolution > leftPhotoResolution)
                    rightPoints += filter.photoResolution.priority;
            }
            else {
        ...
        if (filter.videoAspectRatio != null) {
            ...
            if (leftDiff < rightDiff)
                leftPoints += filter.videoAspectRatio.priority;
            if (rightDiff < leftDiff)
                rightPoints += filter.videoAspectRatio.priority;
        }
        ...
        if (filter.photoAspectRatio != null) {
            ...
            if (leftDiff < rightDiff)
                leftPoints += filter.photoAspectRatio.priority;
            if (rightDiff < leftDiff)
                rightPoints += filter.photoAspectRatio.priority;
        }
        // Find closest max FPS
        if (filter.fps != null) {
            if (filter.fps.target === 'max') {
                if (bestFormat.maxFps > format.maxFps)
                    leftPoints += filter.fps.priority;
                if (format.maxFps > bestFormat.maxFps)
                    rightPoints += filter.fps.priority;
            }
            else {
                if (bestFormat.maxFps >= filter.fps.target)
                    leftPoints += filter.fps.priority;
                if (format.maxFps >= filter.fps.target)
                    rightPoints += filter.fps.priority;
            }
        }
        // Find closest ISO
        if (filter.iso != null) {
            if (filter.iso.target === 'max') {
                if (bestFormat.maxISO > format.maxISO)
                    leftPoints += filter.iso.priority;
                if (format.maxISO > bestFormat.maxISO)
                    rightPoints += filter.iso.priority;
            }
            else if (filter.iso.target === 'min') {
                if (bestFormat.minISO < format.minISO)
                    leftPoints += filter.iso.priority;
                if (format.minISO < bestFormat.minISO)
                    rightPoints += filter.iso.priority;
            }
            else {
                if (filter.iso.target >= bestFormat.minISO && filter.iso.target <= bestFormat.maxISO)
                    leftPoints += filter.iso.priority;
                if (filter.iso.target >= format.minISO && filter.iso.target <= format.maxISO)
                    rightPoints += filter.iso.priority;
            }
        }
        ...
        if (filter.photoHdr != null) {
            if (bestFormat.supportsPhotoHdr === filter.photoHdr.target)
                leftPoints += filter.photoHdr.priority;
            if (format.supportsPhotoHdr === filter.photoHdr.target)
                rightPoints += filter.photoHdr.priority;
        }
        // Find Video HDR formats
        if (filter.videoHdr != null) {
            if (bestFormat.supportsVideoHdr === filter.videoHdr.target)
                leftPoints += filter.videoHdr.priority;
            if (format.supportsVideoHdr === filter.videoHdr.target)
                rightPoints += filter.videoHdr.priority;
        }
        // Find matching AF system
        if (filter.autoFocusSystem != null) {
            if (bestFormat.autoFocusSystem === filter.autoFocusSystem.target)
                leftPoints += filter.autoFocusSystem.priority;
            if (format.autoFocusSystem === filter.autoFocusSystem.target)
                rightPoints += filter.autoFocusSystem.priority;
        }
        // Find depth capture support
        if (filter.supportsDepthCapture != null) {
            if (bestFormat.supportsDepthCapture === filter.supportsDepthCapture.target)
                leftPoints += filter.supportsDepthCapture.priority;
            if (format.supportsDepthCapture === filter.supportsDepthCapture.target)
                rightPoints += filter.supportsDepthCapture.priority;
        }
