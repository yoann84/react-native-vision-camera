...
export class Camera extends React.PureComponent {
    ...
    getBitRateMultiplier(bitRate) {
        if (typeof bitRate === 'number' || bitRate == null)
            return 1;
        switch (bitRate) {
            case 'extra-low':
                return 0.6;
    ...
    startRecording(options) {
        ...
        const onRecordCallback = (video, error) => {
            ...
            if (error != null)
                return onRecordingError(error);
            if (video != null)
                return onRecordingFinished(video);
        };
    ...
    componentDidUpdate() {
        ...
        if (frameProcessor?.frameProcessor !== this.lastFrameProcessor) {
            ...
            if (frameProcessor != null)
                this.setFrameProcessor(frameProcessor.frameProcessor);
            else
